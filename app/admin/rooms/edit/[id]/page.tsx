...
export default function EditRoomPage() {
  ...
  useEffect(() => {
    if (roomId) {
      import('../../actions').then(({ getRoomById }) => {
        getRoomById(roomId).then(data => {
          if (data) {
            setFormData({
              ...
              weekendPrice: data.weekendPrice ? String(data.weekendPrice) : '',
  ...
  const handleSave = async (isDraft: boolean) => {
    ...
    try {
      ...
      if (res.success) {
        alert(isDraft ? 'Draft saved successfully!' : 'Room updated successfully!');
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const { uploadRoomImage } = await import('../../add/actions');
    ...
    <div className="space-y-6 animate-fade-in pb-20">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-3xl font-black text-gray-900 dark:text-white">Edit Room</h1>
            <p className="text-gray-500 dark:text-gray-400 mt-1">Update the details for room {formData.number}.</p>
          </div>
        </div>
