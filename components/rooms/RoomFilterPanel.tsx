...
export default function RoomFilterPanel({ onMobileClose }: FilterProps) {
  ...
  return (
    <div className="bg-gray-800 border-2 border-orange-500/20 rounded-2xl shadow-xl overflow-hidden flex flex-col h-full relative">
      ...
      <div className="p-4 flex-grow mt-2 overflow-y-auto">
        <div className="flex justify-between items-center mb-4">
          ...
          {onMobileClose && (
            <button onClick={onMobileClose} className="lg:hidden p-2 text-gray-500 hover:text-orange-500 rounded-full hover:bg-gray-700 transition-colors">
        ...
        <div className="relative mb-4">
          ...
          <input
            ...
            className="w-full pl-9 pr-3 py-2 text-sm bg-gray-900 border border-gray-700 rounded-xl focus:ring-2 focus:ring-orange-500 focus:border-transparent outline-none transition-all shadow-sm"
          />
        </div>
        ...
        <div className="mb-4">
          ...
          <div className="relative">
            <select 
              ...
              className="w-full text-sm appearance-none bg-gray-900 border border-gray-700 rounded-xl py-2 pl-3 pr-8 focus:ring-2 focus:ring-orange-500 outline-none shadow-sm cursor-pointer"
            >
        ...
        <div className="mb-4 border-t border-gray-700 pt-3">
          <button onClick={() => toggleSection('price')} className="w-full flex justify-between items-center mb-2 group">
            <span className="text-sm font-bold text-gray-200 group-hover:text-orange-500 transition-colors">Price Range (₹)</span>
          ...
          {expandedSections.price && (
            <div className="flex items-center gap-2 mt-2">
              <input type="number" placeholder="Min" value={minPrice} onChange={e => setMinPrice(e.target.value)} className="w-full bg-gray-900 border border-gray-700 rounded-lg py-1.5 px-2 focus:ring-1 focus:ring-orange-500 outline-none text-xs" />
              <span className="text-gray-400 text-xs">-</span>
              <input type="number" placeholder="Max" value={maxPrice} onChange={e => setMaxPrice(e.target.value)} className="w-full bg-gray-900 border border-gray-700 rounded-lg py-1.5 px-2 focus:ring-1 focus:ring-orange-500 outline-none text-xs" />
            </div>
          )}
        </div>
        ...
        <div className="mb-4 border-t border-gray-700 pt-3">
          <button onClick={() => toggleSection('capacity')} className="w-full flex justify-between items-center mb-2 group">
            <span className="text-sm font-bold text-gray-200 group-hover:text-orange-500 transition-colors">Guests</span>
            ...
            <div className="flex flex-wrap gap-2 mt-2">
              {CAPACITIES.map(cap => (
                <button
                  ...
                  className={`w-8 h-8 rounded-full font-bold text-xs flex items-center justify-center transition-all shadow-sm border ${selectedCapacity === String(cap) ? 'bg-orange-500 text-white border-orange-500' : 'bg-gray-900 text-gray-300 border-gray-700 hover:border-orange-300'}`}
                >
                  {cap}{cap === 4 ? '+' : ''}
                </button>
              ))}
        ...
        <div className="mb-4 border-t border-gray-700 pt-3">
          <button onClick={() => toggleSection('amenities')} className="w-full flex justify-between items-center mb-2 group">
            <span className="text-sm font-bold text-gray-200 group-hover:text-orange-500 transition-colors">Features & Amenities</span>
            ...
            <div className="grid grid-cols-2 gap-2 mt-2">
              {AMENITIES.map(am => (
                <label key={am} className="flex items-center gap-2 cursor-pointer group">
                  <div className={`relative w-6 h-3.5 rounded-full transition-colors ${selectedAmenities.includes(am) ? 'bg-orange-500' : 'bg-gray-600'}`}>
                  ...
                  <span className="text-[11px] text-gray-300 line-clamp-1" title={am}>{am}</span>
                </label>
              ))}
            </div>
          )}
        ...
        <div className="mb-4 border-t border-gray-700 pt-3">
          <button onClick={() => toggleSection('view')} className="w-full flex justify-between items-center mb-2 group">
            <span className="text-sm font-bold text-gray-200 group-hover:text-orange-500 transition-colors">Room View</span>
            ...
            <div className="grid grid-cols-2 gap-2 mt-2">
              {VIEW_TYPES.map(view => (
                <button
                  ...
                  className={`text-[11px] py-1.5 px-2 rounded-lg border text-center transition-all ${selectedView === view ? 'bg-orange-900/30 border-orange-500 text-orange-300 font-bold' : 'bg-gray-900 border-gray-700 text-gray-400 hover:border-orange-300'}`}
                >
                  {view}
                </button>
              ))}
        ...
        <div className="pt-3 border-t border-gray-700 mb-4">
          <label className="flex items-center justify-between cursor-pointer group">
            <span className="text-sm font-bold text-gray-200 group-hover:text-orange-500 transition-colors">Show Available Only</span>
            <div className={`relative w-8 h-4 rounded-full transition-colors ${availableOnly ? 'bg-orange-500' : 'bg-gray-600'}`}>
      ...
      {/* Mobile Apply Button */}
      {onMobileClose && (
        <div className="p-4 border-t border-orange-500/20 bg-gray-900 lg:hidden">
